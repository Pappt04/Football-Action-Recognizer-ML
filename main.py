...
# ══════════════════════════════════════════════════════════════════════════════
# FOCAL LOSS
# Zamena za SparseCategoricalCrossentropy.
# Fokusira se na teške primere — smanjuje doprinos lakih (visokonivovskih)
# predikcija. Prirodno redukuje false positive jer model ne može lako da
# "prevari" loss sa visokim confidence-om pogrešne klase.
#   γ (gamma) = 2.0  → standardna vrednost iz originalne paper
# ══════════════════════════════════════════════════════════════════════════════
...
LABEL_TO_IDX = {
    'goal': 0, 'goal_attempt': 1, 'free_kick': 2,
    'offside': 3, 'card': 4, 'normal_play': 5,
}
...
# tf.constant verzije za upotrebu unutar tf.data pipeline-a
...
def preprocess_fn(path, label):
    ...
    img = img / 255.0              # [0, 1]
    img = (img - _TF_MEAN) / _TF_STD  # ImageNet standardizacija (per spec)
    return img, label
...
def build_model():
    """
    EfficientNetB0 (frozen) + GlobalAvgPool + Dense(256) + Dropout(0.5) + Dense(6)
    Vraća (model, backbone) — backbone treba za fine-tuning fazu.
    """
...
def phase2(model, backbone, train_ds, val_ds, class_weights, epochs=EPOCHS_P2):
    ...
    print(f'Trenabilni parametri: {sum(np.prod(v.shape) for v in model.trainable_variables):,}')
    return model.fit(
        train_ds, validation_data=val_ds,
        epochs=epochs, class_weight=class_weights,
...
def evaluate(model, test_ds):
    ...
    # ── Analiza confidence praga ──────────────────────────────────────────────
    # Prikazuje koliko predikcija pada ispod praga sigurnosti (potencijalni FP)
    ...
    for thr in [0.4, 0.5, 0.6, 0.7, 0.8]:
        ...
        # Od "nejasnih" predikcija, koliko je zaista FP
        ...
        if mask_below.sum() > 0:
            ...
            fp_pct = 0.0
        print(f'  conf < {thr:.1f}: {below:4d} primerа ({pct:5.1f}%)  '
...
def plot_training_history(h1, h2):
    ...
    for ax, train, val, title, ylabel in [
        ...
        ax.text(p1_n*0.5,     max(train)*0.97, 'Faza 1', ha='center', color='gray', fontsize=9)
        ax.text(p1_n+(len(ep)-p1_n)*0.5, max(train)*0.97, 'Faza 2', ha='center', color='gray', fontsize=9)
        if ylabel == 'Accuracy':
...
class GradCAM:
    def __init__(self, model, backbone):
        ...
        # Model koji vraća (conv_out @ target sloj, backbone finalni izlaz)
        # Oba izlaza dolaze iz ISTE forward prolaze → gradijent može da teče
    ...
    def compute(self, img_batch, class_idx=None):
        ...
        with tf.GradientTape() as tape:
            # Jedan forward pass: conv_out i backbone_out su iz iste propagacije
            # backbone_out ZAVISI od conv_out → gradijent može da se izračuna
            conv_out, backbone_out = self.backbone_grad_model(img, training=False)
            tape.watch(conv_out)

            # Prolazak kroz head (GAP → Dense → Dropout → Dense)
...
def plot_false_positives(test_paths, y_true, y_pred, y_prob, n_examples=8):
    ...
    # Uzmi n_examples nasumičnih grešaka
    ...
    fig.suptitle('False Positives — Pogrešne predikcije modela',
    ...
    for plot_i, idx in enumerate(selected):
        ...
        # Pozicija unutar ax-a (slika gore, bar chart dole)
        # Koristimo inset_axes za precizno pozicioniranje
        from mpl_toolkits.axes_grid1.inset_locator import inset_axes

        # ── Slika (gornja 55% visine) ──────────────────────────────────────
        ...
        ax.text(0.5, 0.43,
                ...
                bbox=dict(facecolor='#27ae60', edgecolor='none',
                          boxstyle='round,pad=0.3', alpha=0.95))
        ax.text(0.5, 0.35,
                f'✗ MODEL:     {pred_name.upper()}',
                transform=ax.transAxes, ha='center', va='top',
                fontsize=9, fontweight='bold', color='white',
                bbox=dict(facecolor='#e74c3c', edgecolor='none',
                          boxstyle='round,pad=0.3', alpha=0.95))

        # ── Bar chart (donja 30% visine) ───────────────────────────────────
...
def plot_gradcam(gradcam, test_paths, y_true, y_pred, n=3):
    ...
    for row, cls in enumerate(range(NUM_CLASSES)):
        axes[row][0].set_ylabel(IDX_TO_LABEL[cls].upper(), fontsize=8, fontweight='bold')
        idxs = random.sample(per_class.get(cls, []), min(n, len(per_class.get(cls, []))))

        for col_pair, idx in enumerate(idxs):
            raw = tf.io.read_file(test_paths[idx])
            img = tf.image.decode_image(raw, channels=3, expand_animations=False)
            img_255 = tf.image.resize(tf.cast(img, tf.float32), [IMG_SIZE, IMG_SIZE]).numpy()
            img_01  = img_255 / 255.0   # samo za prikaz (originalne boje)

            # Normalizuj isto kao u pipeline-u pre nego što proslediš modelu
            ...
            axes[row][col_pair*2+1].set_title(f'{conf*100:.0f}%', fontsize=8,
                                               backgroundcolor='#e74c3c', color='white', pad=1)
...
def compute_baseline(y_train, y_test, model_acc, model_f1):
    ...
    print('  ' + '-'*50)
    print(f'  {"Majority ("+IDX_TO_LABEL[maj_cls]+")":30s} {acc_maj*100:>8.2f}% {f1_maj:>8.4f}')
...
def main(quick: bool = False, sample_pct: int = 100):
    ...
    # Određivanje efektivnog procenta podataka
    if quick:
        pct = 5
        ep1, ep2 = 2, 1
    ...
    # ── 3. Class weights ──────────────────────────────────────────────────────
    # 'balanced' daje puni inverzni odnos frekfencija (npr. 9x za offside),
    # što pre-agresivno povećava recall malih klasa i kreira mnoštvo FP.
    # Koristimo sqrt(balanced) — blaži kompromis između balansa i preciznosti.
    cw_raw  = compute_class_weight('balanced', classes=np.unique(y_train), y=y_train)
    cw_arr  = np.sqrt(cw_raw)                     # ublaži eksponencijalnu korekiciju
    cw_arr  = cw_arr / cw_arr.min()               # normalizuj: min weight = 1.0
    ...
    # ── Rezultati JSON ────────────────────────────────────────────────────────
    results = {
        'model':    {'backbone': 'EfficientNetB0', 'num_classes': NUM_CLASSES,
                     'total_params': int(best.count_params())},
        'dataset':  {'total': len(paths), 'train': len(X_train),
                     'val': len(X_val), 'test': len(X_test),
                     'class_counts': {IDX_TO_LABEL[i]: int(np.sum(labels==i))
                                      for i in range(NUM_CLASSES)}},
        'test_metrics': {
            'accuracy': float(acc), 'weighted_f1': float(f1_w),
            'per_class_f1': {IDX_TO_LABEL[i]: float(f1_per[i])
                             for i in range(NUM_CLASSES)},
            'goal_accuracy_met': acc > 0.75,
            'goal_f1_met':       f1_w > 0.72,
        },
        'baseline': {
            'majority_accuracy': float(acc_maj), 'majority_f1': float(f1_maj),
            'random_accuracy':   float(acc_rnd), 'random_f1':   float(f1_rnd),
        },
    }
    ...
    # Otvori outputs folder u Finderu (macOS)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sample', type=int, default=100, metavar='PCT',
